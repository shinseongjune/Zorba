...
실제 제작과 Unreal 학습은 [UnrealLearningProductionRoadmap.md](UnrealLearningProductionRoadmap.md)를 함께 사용합니다. 이 체크리스트는 **무엇을 만들지**, 학습 로드맵은 **사용자가 직접 만들 수 있도록 어떤 순서와 역할 분담으로 진행할지**를 관리합니다. 매 제작 세션은 두 파일의 현재 위치를 먼저 확인하고, 완료된 제작 게이트를 학습 명목으로 다시 열지 않습니다.

## 현재 작업 인계 — 2026-09-06 갱신

사용자가 광폭화와 금단 기술 해제를 포함한 전투 게이트와 `L_Boot → M01 → 성공/실패 → 재시작/Boot 복귀` 미션 루프를 승인했고, M01 폴리시도 완료했습니다. UMG에서 목표·남은 적·성공/실패 결과, 재시작·Boot 복귀 버튼과 패드 포커스를 구성했으며 `Development Editor / Win64` 정식 빌드와 사용자 PIE에서 전체 흐름 및 패드 탐색·확정을 통과했습니다. 따라서 기존 전투, M01 루프와 폴리시는 회귀가 생기지 않는 한 다시 열지 않습니다. **현재 활성 게이트는 캐릭터·애니메이션 제품 파이프라인입니다.** 다음 재개 지점은 `D07`로, Marketplace 원본·프로젝트 소유 에셋·파생 에셋의 경계를 확인하고 Reference Viewer, Migrate, Redirector와 폴더 규칙을 사용해 필요한 후보만 좁히는 것입니다. 전용 모션과 SFX/VFX, AI Perception·NavMesh·Behavior Tree/StateTree는 각 후속 게이트에서 처리합니다.
