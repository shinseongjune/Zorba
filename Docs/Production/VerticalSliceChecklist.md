...
## 현재 작업 인계 — 2026-08-01 갱신

`5. 첫 무기 타격`의 기능적 기본 공격 게이트와 `6. 다음 구현 순서`의 **적 기본 공격**, **방어·패리**, **고급 공격·기회공격·처형/회복**, **군중전·적 AI 이동과 2.25초 정예 가드**는 자동 실행 증거와 사용자 PIE 승인까지 끝났으므로 다시 열지 않습니다. 첫 신성한 계율인 후방 공격 패시브도 구현·정식 빌드·전후방 자동 비교까지 끝났지만, 별도 사용자 PIE 승인 증거는 아직 기록하지 않습니다. **2026-08-01 사용자가 다음 단계 진행을 지시해 첫 금단 기술 슬롯 1의 광폭 파훼 기능, 정식 빌드와 맵 자동 실행까지 완료했습니다. 현재 활성 게이트는 첫 금단 기술 사용자 PIE 승인입니다.** 이 승인 뒤 최소 전투 UI와 미션 성공/실패 흐름으로 진행합니다. 전용 모션과 SFX/VFX, AI Perception·NavMesh·Behavior Tree/StateTree, 최종 캐릭터 교체는 기능 계약을 유지한 채 후속 표현·AI 확장·최종 통합 게이트에서 처리합니다.
...
### 현재 확인 요청 — 첫 금단 기술 사용자 PIE 승인

- 정예의 체력을 `50%` 이하로 내리면 머리 위 표시가 `ELITE / ENRAGED`로 바뀌고, 평상시보다 빠르게 접근·공격하며 광폭 중에는 가드를 새로 들지 않는지 확인합니다.
- 광폭 정예를 화면 정면 약 `10m` 안에 둔 뒤 키보드 `1` 또는 패드 `LT + A`를 누르면 보라색 방향 표시와 `STUNNED`가 한 번 나오고, 진행 중 공격과 이동이 즉시 멈추는지 확인합니다.
- `2.5초` 뒤 적이 다시 움직이되 `ENRAGED`로 돌아가지 않는지 확인합니다. 이 기술은 체력·전투 스태미나 피해를 주지 않고 별도 자원도 소비하지 않아야 합니다.
- 성공 직후 재입력은 `8초` 쿨다운으로 거절되어야 합니다. 비광폭 적, 전방 `±55도` 밖, `10m` 밖에서 누른 실패 입력은 적에게 영향을 주지 않고 쿨다운도 시작하지 않아야 합니다.
- 같은 세션에서 첫 신성한 계율의 노란 `SACRED REAR x1.50`, 기회공격/처형, 가드 브레이크가 기존대로인지 함께 보면 남아 있는 계율 사용자 PIE 항목도 닫을 수 있습니다.
