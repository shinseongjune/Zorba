...
이렇게 해야 방어가 LT인지 LB인지, 기술 조합이 어디에 묶였는지 같은 문제가 한 곳에서만 결정됩니다.
...
C++ 캐릭터는 이제 `PrimaryAttack`, `Defend`, `Dodge`, `ProfaneDash`, `UseAbilitySlot1` 같은 입력 이름을 받습니다.

이동, 카메라, 달리기 토글, 회피, 불경한 대쉬, 기술 레이어, 카메라 리셋은 기본 동작이 있습니다. 공격과 기술은 아직 실제 판정이 아니라 로그와 Blueprint 이벤트로 연결되어 있습니다.
...
LT + 얼굴 버튼 기술 조합은 현재 C++에서 기술 레이어 상태를 기억한 뒤 기존 얼굴 버튼 입력을 기술 슬롯으로 돌리는 방식입니다. 그래서 Enhanced Input의 Chorded Action을 쓰지 않아도 먼저 테스트할 수 있습니다.
